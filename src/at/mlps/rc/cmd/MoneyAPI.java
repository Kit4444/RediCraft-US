		...
		if(!(sender instanceof Player)) {
			...
			if(cmd.getName().equalsIgnoreCase("money")) {
					...
					if(args.length == 0) {
						...
						if(p2 == null) {
							...
							if(money < 0) {
								...
								ActionLogger.log(api.getServerName(), p, "Player attempted to use setmoney command, used negative value!");
							}else {
								...
								ActionLogger.log(api.getServerName(), p, "Player used setmoney command, setted money to " + money + " coins for " + p2.getName() + ".");
							}
						}
					}else {
						p.sendMessage(api.prefix("main") + api.returnStringReady(p, "usage") + " §7/setmoney <Player> <Money>");
					...
					if(args.length >= 1 && args.length <= 2) {
						...
						if(p2 == null) {
							...
							if(args[1].matches("^[0-9]+$")) {
								...
								ActionLogger.log(api.getServerName(), p, "Player used addmoney command, added " + money + " coins to" + p2.getName() + " account.");
							}
						}
					}else {
						...
						if(p2 == null) {
							...
							if(args[1].matches("^[0-9]+$")) {
								...
								if(money <= oldmoney) {
									...
									ActionLogger.log(api.getServerName(), p, "Player used removemoney command, removed " + money + " coins from " + p2.getName() + " account.");
								}else {
									api.sendMSGReady(p, "cmd.removemoney.belowzero");
									ActionLogger.log(api.getServerName(), p, "Player attempted to use removemoney command, new value would've been negative.");
								}
							}
						}
					}else {
				...
				if(args.length >= 1 && args.length <= 2) {
					...
					if(p2 == null) {
						...
						if(money <= oldmoney1) {
							...
							ActionLogger.log(api.getServerName(), p, "Player used pay command, " + p2.getName() + " got " + money + " coins.");
							...
							ActionLogger.log(api.getServerName(), p, "Player attempted to use pay command, the value would've been negative.");
						}
					}
				}else {
				...
				try {
					...
					rs.first();
					ActionLogger.log(api.getServerName(), p, "Player used topmoney command. TOP: " + rs.getString("username") + " with " + rs.getInt("money") + " coins");
					...
					if(p.hasPermission("mlps.setbankmoney")) {
						...
						if(p2 == null) {
							...
							if(money <= 0) {
								...
								ActionLogger.log(api.getServerName(), p, "Player used setbankmoney command new bank balance is " + money + " coins.");
							}
						}
					}else {
						api.noPerm(p);
				...
				if(args.length == 1) {
					...
					if(money <= currMoney) {
						...
						ActionLogger.log(api.getServerName(), p, "Player used bankdeposit command, deposed " + money + " coins.");
					}else {
						api.sendMSGReady(p, "cmd.bankdeposit.moreaspossible");
						ActionLogger.log(api.getServerName(), p, "Player attempted to use bankdeposit command, new value would've been negative.");
					}
				}else {
				...
				if(args.length == 1) {
					...
					if(money <= currMoney) {
						...
						ActionLogger.log(api.getServerName(), p, "Player used bankwithdraw command, withdrawed " + money + " coins.");
					}else {
						api.sendMSGReady(p, "cmd.bankwithdraw.moreaspossible");
						ActionLogger.log(api.getServerName(), p, "Player attempted to use bankwithdraw command, new value would've been negative.");
					}
				}else {
